...
class MultiDatabaseSentenceRAGSystem:
    ...
    def query(self, prompt: str, top_k_per_sentence: int = 5, 
        ...
        """
        # print(f"Processing query: {prompt[:100]}...")
        ...
        for i, sentence in enumerate(sentences):
            # print(f"Searching sentence {i+1}: {sentence[:50]}...")
    ...
    def get_context_for_generation_per_sentence(self, results: Dict) -> str:
        """
        NEW METHOD: Create context string from per-sentence results (preserves all chunks)
    ...
    def get_context_for_generation_flat(self, results: Dict) -> str:
        """
        NEW METHOD: Create context string from all chunks in flat list (preserves duplicates)
        ...
        for i, chunk in enumerate(results['all_chunks_flat'], 1):
            context_parts.append(f"\n[Chunk {i} - {chunk.database_name}:{chunk.source}]")
    ...
    def print_results(self, results: Dict, show_content: bool = True, max_content_length: int = 500):
        ...
        print("\n" + "="*80)
        print("MULTI-DATABASE SENTENCE-LEVEL RAG SEARCH RESULTS")
        print("="*80)
        ...
        if 'all_retrieved_chunks' in results:
            ...
            # Per-sentence method without deduplication
            print(f"\n{'='*60}")
            print(f"ALL CHUNKS RETRIEVED ({len(results['all_chunks_flat'])} total, duplicates preserved)")
            print(f"{'='*60}")
            print("Note: Use get_context_for_generation_per_sentence() or get_context_for_generation_flat() for context generation")
    
    def get_context_for_generation(self, results: Dict, max_chunks: int = 10) -> str:
        """Create a context string from top chunks for text generation (ORIGINAL METHOD)"""
        if 'all_retrieved_chunks' in results:
            # Original method with deduplication
            top_chunks = results['all_retrieved_chunks'][:max_chunks]
        else:
            # Per-sentence method - take first max_chunks from flat list
            top_chunks = results['all_chunks_flat'][:max_chunks]
...
def search_per_sentence_topk(rag_system: MultiDatabaseSentenceRAGSystem, prompt: str, 
    ...
    """
    NEW FUNCTION: Search a prompt and get top-k chunks per sentence (no global deduplication)
...
results = search_per_sentence_topk(rag_system, "Your query here", top_k=5)

# Get context for generation (organized by sentence)
context = rag_system.get_context_for_generation_per_sentence(results)

# Or get flat context (all chunks in one list)
context_flat = rag_system.get_context_for_generation_flat(results)

# ORIGINAL: Search with global deduplication (still available)
results_dedupe = search_across_databases(rag_system, "Your query here", top_k=5)
"""
# import json
# import re
# import numpy as np
...
#         # Overall top chunks with database info
#         print(f"\n{'='*60}")
#         print("TOP UNIQUE CHUNKS ACROSS ALL SENTENCES")
#         print(f"{'='*60}")
...
#         top_chunks = results['all_retrieved_chunks'][:max_chunks]
#         # print("Chunks Retrieved:",len(top_chunks))
#         context_parts = []
...
#                            print_results: bool = True) -> Dict:
#     """
#     Search a prompt across multiple databases
...
# # Example usage:
# """
# # Method 1: List of files
# chunk_files = [
#     '../Doc_RAG/nswh_output/chunks/chunks.json',
#     '../Doc_RAG/other_db/chunks/chunks.json',
#     # ... more files
# ]
# rag_system = create_multi_rag_system(chunk_files)

# # Method 2: Directory containing JSON files
# rag_system = create_multi_rag_system('../Doc_RAG/all_chunks_directory/')

# # Method 3: Direct instantiation
# rag_system = MultiDatabaseSentenceRAGSystem(chunks_files=chunk_files)

# # Search across all databases
# results = search_across_databases(rag_system, "Your query here", top_k=5)

# # Search within specific database
# results = search_across_databases(rag_system, "Your query here", search_mode="chunks", top_k=5)
