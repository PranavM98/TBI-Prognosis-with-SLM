...
from partB_prompts_RAG import prompt_return, prompt_one
import time
import os
import re
import requests
...
rag_system = doc_rag_multiple.MultiDatabaseSentenceRAGSystem(chunks_files=chunk_files)
...
def save_results(dataframe, args):
    ...
    filename = f"{args.model}_{file_suffix}.csv" if args.type == 'few' else f"{args.model}_k0.csv"


    folderpath='full_results_RAGpartB/final/'
...
def process_patient_data(args, data, test_list=5):
    ...
    for i in tqdm(range(test_list)):
        ...
        results = doc_rag_multiple.search_per_sentence_topk(rag_system, final_answer, top_k=3)
...
if __name__ == "__main__":
    ...
    # Load data
    data = pd.read_csv("/cwork/pm231/data_patients_partB.csv")
